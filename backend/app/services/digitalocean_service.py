...
class DigitalOceanService:
    ...
    def get_account_complete_info(self) -> Dict[str, Any]:
        """Get complete account information with droplets and other resources"""
        if not self.client:
            raise Exception("DigitalOcean API client not available. Please check your API token.")
        
        try:
            logger.info(f"🔄 Fetching complete account info from DigitalOcean API")
            
            # Get basic account info
            account_info = self.get_account()
            
            # Get droplets count
            droplets_info = self.get_droplets()
            droplets_count = len(droplets_info.get('droplets', []))
            
            # Combine information
            complete_info = {
                "account": account_info.get('account', {}),
                "droplets_count": droplets_count,
                "droplets": droplets_info.get('droplets', []),
                "links": {},
                "meta": {"total_droplets": droplets_count}
            }
            
            logger.info(f"✅ SUCCESS! Fetched complete account info with {droplets_count} droplets")
            return complete_info
            
        except Exception as e:
            logger.error(f"❌ DigitalOcean API Error: {e}")
            raise Exception(f"Failed to fetch complete account info from DigitalOcean: {e}")
