...
const CreateDroplet: React.FC = () => {
  ...
  if (!hasValidTokens) {
    return (
      <div className="container mx-auto px-4 py-8">
        <NoTokenMessage 
          message="Bạn cần cấu hình DigitalOcean API token để tạo VPS. Vào Settings để thêm token."
        />
      </div>
    )
  }
  ...
  const createMutation = useMutation({
    ...
    onError: (error: any) => {
      toast.error(error.response?.data?.detail || 'Failed to create droplet')
    }
  })
  ...
  return (
    <div className="space-y-6 p-6">
      {/* Header */}
      <div>
        <h1 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>Create New Droplet</h1>
        <p className={`mt-1 text-sm ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
          Deploy a new Windows VPS instance
        </p>
      </div>
              ...
              <div className="px-4 py-5 sm:p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      {...register('name', { 
                        required: 'Droplet name is required',
                  ...
                  <div>
                    ...
                    {imagesLoading && (
                      <p className={`mt-1 text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Loading images from DigitalOcean...</p>
                    )}
                    {!imagesLoading && images.length === 0 && (
                      <p className={`mt-1 text-sm ${isDark ? 'text-yellow-400' : 'text-yellow-600'}`}>No images available</p>
                    )}
                  </div>
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  ...
                  <div className={`mt-2 text-sm ${isDark ? 'text-blue-200' : 'text-blue-700'}`}>
                    <p>
                      Your Windows droplet will be automatically configured with:
                    </p>
